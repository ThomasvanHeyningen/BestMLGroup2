...
#   clf = AdaBoostClassifier(RandomForestClassifier(n_jobs=3, n_estimators=200, max_depth=15))
    ...
    print('RFC 1 LogLoss {score}'.format(score=log_loss(y_valid, clf3.predict_proba(X_valid))))
    print('RFC 1 accuracy {score}'.format(score=accuracy_score(y_valid, clf3.predict(X_valid))))
    clfs.append(clf3)
    ...
    print('GBM LogLoss {score}'.format(score=log_loss(y_valid, gbm3.predict_proba(X_valid))))
    print('GBM accuracy {score}'.format(score=accuracy_score(y_valid, gbm3.predict(X_valid))))
    clfs.append(gbm3)
    ...
    print('GBM 2 LogLoss {score}'.format(score=log_loss(y_valid, gbm4.predict_proba(X_valid))))
    print('GBM 2 accuracy {score}'.format(score=accuracy_score(y_valid, gbm4.predict(X_valid))))
    clfs.append(gbm4)

    clf4 = RandomForestClassifier(n_jobs=3, n_estimators=200, max_depth=20, random_state=288)
    clf4.fit(X_train, y_train)
    print('RFC 2 LogLoss {score}'.format(score=log_loss(y_valid, clf4.predict_proba(X_valid))))
    print('RFC 2 accuracy {score}'.format(score=accuracy_score(y_valid, clf4.predict(X_valid))))
    clfs.append(clf4)
