...
#   clf = AdaBoostClassifier(RandomForestClassifier(n_jobs=3, n_estimators=200, max_depth=15))
    ...
    clf3 = RandomForestClassifier(n_jobs=3, n_estimators=200, max_depth=27, random_state=280)
    clf3.fit(X_train, y_train)
    print('RFC 3 LogLoss {score}'.format(score=log_loss(y_valid, clf3.predict_proba(X_valid))))
    print('RFC 3 accuracy {score}'.format(score=accuracy_score(y_valid, clf3.predict(X_valid))))
    clfs.append(clf3)

    gbm3=GradientBoostingClassifier(n_estimators=50, max_depth=18, max_features=15, min_samples_leaf=4,verbose=1, subsample=0.70, random_state=287)
    gbm3.fit(X_train, y_train)
    print('GBM 3 LogLoss {score}'.format(score=log_loss(y_valid, gbm3.predict_proba(X_valid))))
    print('GBM 3 accuracy {score}'.format(score=accuracy_score(y_valid, gbm3.predict(X_valid))))
    clfs.append(gbm3)

    gbm4=GradientBoostingClassifier(n_estimators=50, max_depth=12, max_features=25, min_samples_leaf=4,verbose=1, subsample=0.95, random_state=286)
    gbm4.fit(X_train, y_train)
    print('GBM 4 LogLoss {score}'.format(score=log_loss(y_valid, gbm4.predict_proba(X_valid))))
    print('GBM 4 accuracy {score}'.format(score=accuracy_score(y_valid, gbm4.predict(X_valid))))
    clfs.append(gbm4)

    clf4 = RandomForestClassifier(n_jobs=3, n_estimators=200, max_depth=20, random_state=288)
    clf4.fit(X_train, y_train)
    print('RFC 4 LogLoss {score}'.format(score=log_loss(y_valid, clf4.predict_proba(X_valid))))
    print('RFC 4 accuracy {score}'.format(score=accuracy_score(y_valid, clf4.predict(X_valid))))
    clfs.append(clf4)
